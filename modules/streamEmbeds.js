...
function buildStreamEmbed({ platform, username, avatarUrl, url, title, game, imageUrl }) {
  ...
  const platformMeta = meta[platform] || { color: 0x2f3136, name: 'Stream', icon: null };

  const embed = new EmbedBuilder()
    .setColor(platformMeta.color)
    .setAuthor({ name: `${username || 'Streamer'}`, iconURL: avatarUrl || platformMeta.icon, url })
    .setTitle(title || 'Live now')
    .setURL(url)
    .setDescription(game ? `Activity: ${game}` : null)
    .setImage(imageUrl || avatarUrl || platformMeta.icon);
